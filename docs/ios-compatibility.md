...
| Widget data and timer | Offline snapshot of 30 days, independent of notification permissions. Elapsed time for at most one hour after a prayer, switching earlier if the next prayer is at most one hour away; countdown red strictly below one hour. System timer handles seconds; timeline handles phase changes. The displayed date follows the displayed prayer and the large schedule keeps a late Isha with its original prayer day. Minute countdowns round remaining time up, matching Android. WidgetKit controls refresh timing. This source change is newer than App Store build 32. |
...
| Automatic location changes | The opted-in iOS position stream uses low accuracy and a 1,500 metre filter. Background updates and the system location indicator are enabled only with Always permission; otherwise an allowed stream is foreground-only. Prayer recalculation still requires 3,000 metres of movement. No continuation after force-quit is guaranteed. |
