...
bool backgroundEnabled(LocationSettings settings) => switch (settings) {
  AppleSettings() => settings.allowBackgroundLocationUpdates,
  AndroidSettings() => settings.foregroundNotificationConfig != null,
  _ => false,
};
    ...
    group(platform.name, () {
      ...
      test(
        ...
        () async {
          ...
          expect(backgroundEnabled(locations.settings.single), isFalse);
          always = true;
          await LocationAutoUpdateService.start();
          expect(locations.cancellations, 1);
          expect(backgroundEnabled(locations.settings.last), isTrue);
          await Future.wait([
      ...
      test(
        ...
        () async {
          ...
          always = false;
          await LocationAutoUpdateService.start();
          expect(backgroundEnabled(locations.settings.last), isFalse);
          foreground = false;
          await LocationAutoUpdateService.start();
      ...
      test(
        ...
        () async {
          ...
          await prefs.setDouble('auto_location_last_lng', -5);
          final prayer =
              Get.put<PrayerTimeController>(_Prayer(prefs)) as _Prayer;
          await LocationAutoUpdateService.start();
