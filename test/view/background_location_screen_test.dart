...
void main() {
  ...
  setUp(() {
    ...
    messenger.setMockMethodCallHandler(
      const MethodChannel('flutter.baseflow.com/geolocator_updates_apple'),
      (_) async => null,
    );
    requested = [];
    alwaysStatus = null;
  ...
  test(
    'iOS onboarding does not require optional background permission',
    () async {
  ...
  testWidgets(
    'neutral pre-prompt has one action; refusal exits without Settings or retry',
    (tester) async {
      await openExplanation(tester);
      expect(find.text(french['activate']!), findsNothing);
      expect(find.text(french['no_thanks']!), findsNothing);
      ...
      expect(prefs.getBool(LocationAutoUpdateService.enabledKey), isNot(true));
      debugDefaultTargetPlatformOverride = null;
    },
  );
